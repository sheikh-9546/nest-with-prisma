...
export class UserService {
  ...
  async getUserDetails(
    ...
    const serializedUser = SerializerUtil.serialize(user, UserDetailsSerializer);
    return {
      ...serializedUser,
  ...
  async getAllUsers(page: number = 1, limit: number = 10, sort_column: string, sort_direction: string): Promise<PaginationResult<any>> {
    ...
    const serializedUsers = users.map((user) => SerializerUtil.serialize(user, UserSerializer));
  ...
  async createUser(data: Prisma.UserCreateInput & { roleId: number }): Promise<any> {
    ...
    if (!roleExists) {
      throw new ConflictException(Messages.Role.Error.IS_ROLE_NOT_FOUND(roleId));
    }

    // Check if the email is already in use
    if (emailExists) {
      throw new ConflictException(Messages.User.Error.EMAIL_ALREADY_EXISTS(email));
    }

    // Check if the phone is already in use
    if (phoneNumberExists) {
      throw new ConflictException(Messages.User.Error.PHONE_NUMBER_ALREADY_EXISTS(phoneNumber));
    }
  ...
  // Assign the role to the user
  private async createUserRole(userId: string, roleId: number): Promise<any> {
  ...
  async updateUser(params: { where: Prisma.UserWhereUniqueInput; data: Prisma.UserUpdateInput; }): Promise<any> {
    const updateUser = await this.prisma.user.update({ where: params.where, data: params.data, });
  ...
  async deleteUser(where: Prisma.UserWhereUniqueInput): Promise<User> {
    ...
    if (!user) {
      ErrorHandler.userNotFoundError(Messages.User.Error.USER_NOT_FOUND, HttpStatus.NOT_FOUND);
    }
  ...
  async changePassword(userId: string, currentPassword: string, newPassword: string) {
    ...
    if (!isPasswordValid) {
      throw new ConflictException(Messages.User.Error.CURRENT_PASSWORDS_DO_NOT_MATCH);
    }
